...
const getPawnMoves = (isWhite, row, col, squares) => {
  ...
  const getPawnForwardMoves = () => {
    ...
    if (squares[getIdx(forwardRow, col)].piece.type === pieceTypes.ES) {
      moves.push(getSquareNameFromRowCol(forwardRow, col));
    }
    // two squares ahead empty, but only on first move
    if (row === startRow && squares[getIdx(forwardTwoRow, col)].piece.type === pieceTypes.ES) {
      moves.push(getSquareNameFromRowCol(forwardTwoRow, col));
    }
  };

  // four possible moves - one forward, two forward, capture left or right (no en passant)
  const getPawnAttackMoves = () => {
    ...
    if (forwardLeftType !== null && forwardLeftType !== pieceTypes.ES && isEnemyPiece(forwardLeftType, isWhite)) {
      moves.push(getSquareNameFromRowCol(forwardRow, forwardLeftCol));
    }
    if (forwardRightType !== null && forwardRightType !== pieceTypes.ES && isEnemyPiece(forwardRightType, isWhite)) {
      moves.push(getSquareNameFromRowCol(forwardRow, forwardLeftCol));
    }
  };
